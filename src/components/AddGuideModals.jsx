import { X, Link as LinkIcon, Table, Video, Quote, Code, Image as ImageIcon, ChevronDown, Type, Tag, Keyboard, Anchor, FileText, Clock, ListOrdered, Download, AlertTriangle, BookOpen, Music, CheckSquare, GitBranch, Layers, Play, File, AlertCircle, ArrowRight, Hash, Calculator, GitMerge, Calendar, MapPin, User, Mail, Phone, Globe } from "lucide-react";
import { useState } from "react";

function ModalContainer({ children, title, onClose }) {
  return (
...
export function LinkModalForm({ onInsert, onClose }) {
...
export function TableModalForm({ onInsert, onClose }) {
...
export function VideoModalForm({ onInsert, onClose }) {
...
export function CalloutModalForm({ onInsert, onClose }) {
...
export function CodeModalForm({ onInsert, onClose }) {
...
export function FigureModalForm({ onInsert, onClose }) {
  const [url, setUrl] = useState("");
...
export function DetailsModalForm({ onInsert, onClose }) {
...
export function QuoteModalForm({ onInsert, onClose }) {
...
export function BadgeModalForm({ onInsert, onClose }) {
  const [text, setText] = useState("");
...
export function KbdModalForm({ onInsert, onClose }) {
...
export function CTAModalForm({ onInsert, onClose }) {
...
export function CitationModalForm({ onInsert, onClose }) {
...
export function AnchorModalForm({ defaultSlug, onInsert, onClose }) {
...
export function FootnoteModalForm({ onInsert, onClose }) {
...
export function StepsModalForm({ onInsert, onClose }) {
  const [steps, setSteps] = useState([{ title: "", content: "" }]);

  const addStep = () => setSteps([...steps, { title: "", content: "" }]);
...
// Timeline Modal
export function TimelineModalForm({ onInsert, onClose }) {
  const [items, setItems] = useState([{ date: "", event: "" }]);

  const addItem = () => setItems([...items, { date: "", event: "" }]);
...
export function ComparisonModalForm({ onInsert, onClose }) {
  ...
  const updateValue = (featureIdx, optionIdx, val) => {
...
// Alert/Notice Modal
export function AlertModalForm({ onInsert, onClose }) {
  const [type, setType] = useState("info");
...
// Tabs Modal
export function TabsModalForm({ onInsert, onClose }) {
  const [tabs, setTabs] = useState([{ label: "Tab 1", content: "" }]);

  const addTab = () => setTabs([...tabs, { label: `Tab ${tabs.length + 1}`, content: "" }]);
...
export function DefinitionModalForm({ onInsert, onClose }) {
  const [items, setItems] = useState([{ term: "", definition: "" }]);

  const addItem = () => setItems([...items, { term: "", definition: "" }]);
  const updateItem = (idx, field, value) => {
    const newItems = [...items];
    newItems[idx][field] = value;
    setItems(newItems);
...
export function CodeDiffModalForm({ onInsert, onClose }) {
...
// FAQ Modal
export function FAQModalForm({ onInsert, onClose }) {
...
export function VersionModalForm({ onInsert, onClose }) {
  const [versions, setVersions] = useState([{ version: "1.0", date: "", changes: "" }]);

  const addVersion = () => setVersions([...versions, { version: `${versions.length + 1}.0`, date: "", changes: "" }]);
...
export function KeyValueModalForm({ onInsert, onClose }) {
