import { Code, FileText, Loader2, Save, X, Eye, BookOpen, Sparkles, Wand2, ChevronUp } from 'lucide-react'
...
export default function AddGuideModal({ onClose }) {
    ...
    if (!isAuthenticated() || !user) {
        return (
            <div className="fixed inset-0 z-[9999] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200" onClick={onClose}>
                <div className="bg-white rounded-lg shadow-2xl p-12 max-w-md w-full text-center animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (response.status === 504) {
                    throw new Error('AI service is taking too long to respond. Please try again with shorter content.')
                }

                const errData = await response.json().catch(() => ({}))
                throw new Error(errData.error || `AI request failed (Status: ${response.status})`)
            }
    ...
    async function handleEnhanceContent() {
        if (!formData.content.trim() && !formData.html_content.trim()) {
            setErrorModal({ show: true, message: 'Please write some content first before enhancing!' })
            return
        }
        ...
        try {
            const contentToEnhance = activeTab === 'markdown' ? formData.content : formData.html_content
...
Enhanced Content:`
            ...
            setErrorModal({ show: true, message: 'Failed to enhance content: ' + err.message })
        } finally {
    ...
    async function handleGenerateContent() {
        if (!formData.title.trim()) {
            setErrorModal({ show: true, message: 'Please enter a title first so AI knows what to generate!' })
            return
        }
...
Generated Content:`
            ...
            setErrorModal({ show: true, message: 'Failed to generate content: ' + err.message })
        } finally {
            setAiProcessing(false)
        }
    }
    ...
    async function handleSubmit(e) {
        ...
        if (!formData.content.trim() && !formData.html_content.trim()) {
            setErrorModal({ show: true, message: 'Please enter content (Markdown or HTML)' })
            return
        }
        ...
        try {
            ...
            setErrorModal({ show: true, message: 'حدث خطأ أثناء الحفظ: ' + (err.message || 'حاول مرة أخرى') })
        } finally {
            setSaving(false)
        }
    }
    ...
    return (
        <>
            {/* Error Modal */}
            {errorModal.show && (
                <div className="fixed inset-0 z-[10000] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200" onClick={() => setErrorModal({ show: false, message: '' })}>
                    <div className="bg-white rounded-xl shadow-2xl p-8 max-w-md w-full text-center animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
            ...
            {/* Main Modal */}
            <div className="fixed inset-0 z-[9999] bg-black/40 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
                {/* Full Screen Modal */}
                <div className="h-screen flex flex-col bg-white" onClick={(e) => e.stopPropagation()}>
                    {/* Header */}
                    <div className="border-b border-gray-200 bg-white">
                        <div className="max-w-[1800px] mx-auto px-8 py-5 flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h1 className="text-xl font-bold text-gray-900">Create New Guide</h1>
                    ...
                    <div className="flex-1 overflow-hidden bg-gray-50">
                        <div className="max-w-[1800px] mx-auto h-full flex gap-6 p-6">
                            {/* Left Side - Form */}
                            <div className={`${showPreview ? 'w-1/2' : 'w-full max-w-4xl mx-auto'} bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden flex flex-col transition-all duration-300 relative`}>
                                <form onSubmit={handleSubmit} className="flex-1 flex flex-col">
                                    <div className="flex-1 overflow-y-auto p-8 space-y-6">
                                        ...
                                        <div>
                                            ...
                                            />
                                            <p className="text-xs text-gray-500 mt-2">Help others discover your guide</p>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <div className="inline-flex rounded-lg border border-gray-300 p-1 bg-gray-50">
                                                <button
                                                    ...
                                                    className={`flex items-center gap-2 px-4 py-2 rounded-md font-medium text-sm transition-all ${activeTab === 'markdown'
                                                ...
                                                <button
                                                    ...
                                                    className={`flex items-center gap-2 px-4 py-2 rounded-md font-medium text-sm transition-all ${activeTab === 'advanced'
                                                        ? 'bg-white text-gray-900 shadow-sm'
                                                        : 'text-gray-600 hover:text-gray-900'
                                                        }`}
                                                >
        ...
        <p>Your content here...</p>
                                                ...
                                                <div className="mt-3 p-3 bg-gray-50 border border-gray-200 rounded-lg">
                                                    <p className="text-xs text-gray-600">
                                                        💡 <strong>Tip:</strong> Paste complete HTML including &lt;style&gt; and &lt;script&gt; tags. Full JavaScript support!
                                                    </p>
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        <div className="mb-3 bg-white border-2 border-black rounded-lg shadow-2xl overflow-hidden animate-in slide-in-from-bottom-2 duration-200">
                                            ...
                                            >
                                                ...
                                                <div className="text-left">
                                                    <div className="font-semibold text-sm">Enhance Content</div>
                                                    <div className="text-xs text-gray-500">Improve clarity & grammar</div>
                                                </div>
                                            </button>
                                            <button
                                                type="button"
                                            ...
                                            >
                                                ...
                                                <div className="text-left">
                                                    ...
                                                    <div className="text-xs text-gray-500">Create from title & keywords</div>
                                                </div>
                                            </button>
                                        </div>
                                    )}
                                    ...
                                    >
                                        {aiProcessing ? (
                                            ...
                                            <>
                                                ...
                                                <ChevronUp size={18} className={showAIMenu ? 'rotate-180' : ''} />
                                            </>
                                        )}
                                    </button>
                                </div>
