...
export default function FollowButton({ targetUserEmail, targetUserName, className = '' }) {
  const { user } = useAuth()
    ...
    return (
      <div className="flex items-center gap-3">
        <div className="text-sm text-gray-600">
          <span className="font-bold text-black">{followersCount}</span> Follower{followersCount !== 1 ? 's' : ''}
        </div>
      </div>
    )
  }
