...
const SocialIcon = ({ name, icon, staticIcon: StaticIcon, isSelected, onClick }: SocialIconProps) => {
...
export default function AccountSetupModal({ user, onClose, onComplete }: AccountSetupModalProps) {
    ...
    const [_bonusCredits, _setBonusCredits] = useState(0)
        ...
        return (
            <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 gap-4 max-h-[400px] overflow-y-auto pr-2 pb-2">
                    {socialSources.map(({ name, icon, staticIcon }) => (
                        <SocialIcon
                            key={name}
                            name={name}
                            icon={icon}
