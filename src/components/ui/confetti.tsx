...
interface ConfettiHandle {
    fire: (options?: { particleCount?: number; origin?: { x: number; y: number } }) => void;
}

export const Confetti = forwardRef<ConfettiHandle, ConfettiProps>(
    ({ className, onMouseEnter, ...props }, ref) => {
        const containerRef = useRef<HTMLDivElement>(null);
        const particlesRef = useRef<Array<ConfettiParticleObj & { el: HTMLElement }>>([]);
        ...
        const createParticle = useCallback((x: number, y: number): ConfettiParticleObj => {
            return {
                id: Math.random(),
        ...
        const fire = useCallback(({ particleCount = 100, origin = { x: 0.5, y: 0.5 } } = {}) => {
            ...
            // Create particles
            const newParticles: ConfettiParticleObj[] = [];
            ...
            const animate = () => {
                ...
                particlesRef.current = particlesRef.current.filter(particle => {
                    ...
                    particle.el.style.opacity = particle.opacity.toString();
