...
const blurFadeImages: string[] = Array.from({ length: 9 }, (_, i) => {
...
const AuthPage: FC = () => {
        ...
        const { data: { subscription } } = supabase.auth.onAuthStateChange(
            (event, session) => {
                ...
                if (
                    ...
                    login(session.access_token, session.user as any);
                ...
                supabase.auth
                    ...
                    .then(({ data, error }) => {
                        if (error) {
                            ...
                            login(data.session.access_token, data.session.user as any);
            ...
            switch (mode) {
                case "register":
                    ...
                    try {
                        ...
                        const errorMsg = e instanceof Error ? e.message : "Network error during registration";
                        throw new Error(errorMsg);
                    }

                    if (!response.ok) {
                        const errorResult = result as Record<string, unknown>;
                        throw new Error((errorResult.error as string) || "Registration failed");
                    }
                ...
                case "login":
                    ...
                    if (signInData.user && signInData.session) {
                        login(signInData.session.access_token, signInData.user as any);
                        navigate("/");
                    }
                    break;
    ...
    const handleGitHubLogin = async (): Promise<void> => {
        try {
            ...
            const errorMsg = error instanceof Error ? error.message : "Failed to connect with GitHub";
            setMessage({
                type: "error",
                text: errorMsg,
            });
