        ...
        try {
            ...
            try {
                const { data: uniqueUsers } = await supabase
                    .from('guides')
                    .select('user_email')
                usersCount = uniqueUsers ? [...new Set(uniqueUsers.map((u: { user_email: string }) => u.user_email).filter(Boolean))].length : 0
            } catch (e: unknown) {
            ...
            try {
                ...
                if (!error && recentGuides) {
                    setRecentActivity(recentGuides.map((g: { title: string; created_at: string; user_email: string }) => ({
                        type: 'guide',
                        title: g.title,
                        time: g.created_at,
                        author: g.user_email
                ...
                <div className="admin-grid">
                    ...
                    <section className="admin-card">
                        ...
                        <div className="quick-actions">
                            ...
                            <Link href="/guides" className="action-btn">
                                <Eye size={18} />
                                View All Guides
                            </Link>
                            <Link href="/support" className="action-btn">
